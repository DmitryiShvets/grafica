...
namespace blank.Render
{
    internal class Scene
    {
        ...
        public Scene()
        {
            ...
            objects.Add(new Box(new Vector4(0, 0, 0), new Vector4(1, 2, 1), new Vector4(2,-3,1), Color.Yellow));

            light_sources = new List<LightSource>();
            light_sources.Add(new LightSource(LIGHT_TYPE.AMBIENT, 0.2, null));
            light_sources.Add(new LightSource(LIGHT_TYPE.POINT, 0.6, new Vector4(1, 1, 1)));
            light_sources.Add(new LightSource(LIGHT_TYPE.DIRECTIONAL, 0.5, new Vector4(0, 0, -1)));
        }
    }
}
